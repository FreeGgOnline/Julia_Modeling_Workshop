...
In this notebook, we'll demonstrate how Universal Differential Equations (UDE) can help discover missing physics in mathematical models. We'll use machine learning to extend partially known dynamical systems and demonstrate the technique using the Lotka-Volterra predator-prey equations.

## Key Concepts

- **Universal Differential Equations (UDE)**: Hybrid models that combine known physics with neural networks
- **Scientific Machine Learning**: Using ML to discover missing terms in scientific models
- **Symbolic Regression**: Converting learned neural network dynamics back into interpretable equations
...
First, we'll generate synthetic data from the true Lotka-Volterra system. This will serve as our "experimental observations" that we'll use to train our hybrid model.
"""
...
# True Lotka-Volterra dynamics
function lotka_volterra_true!(du, u, p, t)
    x, y = u
    α, β, δ, γ = p
    du[1] = α * x - β * x * y
    du[2] = -δ * y + γ * x * y
...
# Initial conditions
u0_true = [0.44249296, 4.6280594]

# ╔═╡ def45678-9abc-0123-4567-89abcdef0123
# Time span for data generation
tspan_data = (0.0, 3.0)

# ╔═╡ ef456789-abcd-1234-5678-9abcdef01234
t_data = range(tspan_data[1], tspan_data[2], length=30)

# ╔═╡ d4e5f6a7-b890-1234-def0-234567890123
# Solve the true system
prob_true = ODEProblem(lotka_volterra_true!, u0_true, tspan_data, p_true)

# ╔═╡ f5678901-bcde-2345-6789-abcdef012345
solution_true = solve(prob_true, Tsit5(), saveat=t_data)

# ╔═╡ a6789012-cdef-3456-789a-bcdef0123456
# Extract data and add noise
X_noiseless = Array(solution_true)

# ╔═╡ b789a123-def0-4567-89ab-cdef01234567
noise_level = 5e-3

# ╔═╡ c89ab234-ef01-5678-9abc-def012345678
X_data = X_noiseless + noise_level * randn(size(X_noiseless))

# ╔═╡ d9abc345-f012-6789-abcd-ef0123456789
# Plot the data
plot_data = plot(solution_true, alpha=0.3, label=["True Prey" "True Predator"],
                 title="Training Data from Lotka-Volterra System")

# ╔═╡ eabcd456-0123-789a-bcde-f01234567890
scatter!(plot_data, t_data, X_data', label=["Noisy Prey Data" "Noisy Predator Data"],
         markersize=2)
...
But we don't know the interaction terms (predation).
"""

# ╔═╡ f6a7b890-1234-5678-f012-456789012345
# Define neural network for missing physics
rbf(x) = exp.(-(x .^ 2))

# ╔═╡ fbcde567-1234-89ab-cdef-012345678901
# Manually set up NN layers for better control
U_nn = Lux.Chain(
    Lux.Dense(2, 5, rbf),
    Lux.Dense(5, 5, rbf),
    Lux.Dense(5, 5, rbf),
...
rng_nn = Random.default_rng()

# ╔═╡ bdef0789-3456-abcd-ef01-234567890123
Random.seed!(rng_nn, 1234)

# ╔═╡ cef01890-4567-bcde-f012-345678901234
nn_params, nn_states = Lux.setup(rng_nn, U_nn)

# ╔═╡ df012901-5678-cdef-0123-456789012345
# Flatten parameters for optimization
p_nn_init = ComponentArray(nn_params)

# ╔═╡ a7b89012-3456-789a-0123-567890123456
# Known parameters (only growth and death rates)
p_known = [1.3, 0.8]  # α and δ

# ╔═╡ e0123a12-6789-def0-1234-567890123456
# Define UDE with partially known dynamics
function ude_dynamics!(du, u, p, t, p_known_local)
    α, δ = p_known_local

    # Neural network prediction for missing terms
    nn_out = U_nn(u, p, nn_states)[1]
...
# ╔═╡ f1234b23-789a-ef01-2345-678901234567
# Create closure for the ODE
ude_f!(du, u, p, t) = ude_dynamics!(du, u, p, t, p_known)

# ╔═╡ a2345c34-89ab-f012-3456-789012345678
# Initial neural network parameters
p_ude_init = p_nn_init
...
function predict_ude(θ)
    prob_ude = ODEProblem(ude_f!, X_data[:, 1], tspan_data, θ)
    sol_ude = solve(prob_ude, Tsit5(), saveat=t_data,
...
function callback_ude(p, l)
    push!(losses_ude, l)
    if length(losses_ude) % 50 == 0
        println("Current loss after $(length(losses_ude)) iterations: $(losses_ude[end])")
    end
    return false
end
...
optf_adam = Optimization.OptimizationFunction((x, p) -> loss_ude(x), adtype)

# ╔═╡ f7890189-def0-4567-89ab-234567890123
optprob_adam = Optimization.OptimizationProblem(optf_adam, p_ude_init)

# ╔═╡ a890129a-ef01-5678-9abc-345678901234
result_adam = Optimization.solve(optprob_adam, ADAM(0.1),
                                 callback=callback_ude, maxiters=200)

# ╔═╡ b90123ab-f012-6789-abcd-456789012345
println("ADAM optimization completed. Final loss: $(losses_ude[end])")

# ╔═╡ f2345678-9abc-def0-5678-012345678901
# Stage 2: L-BFGS optimization for fine-tuning
optf_lbfgs = Optimization.OptimizationFunction((x, p) -> loss_ude(x), adtype)

# ╔═╡ ca0123bc-0123-789a-bcde-567890123456
optprob_lbfgs = Optimization.OptimizationProblem(optf_lbfgs, result_adam.u)

# ╔═╡ db1234cd-1234-89ab-cdef-678901234567
result_lbfgs = Optimization.solve(optprob_lbfgs,
                                  Optim.LBFGS(m=20),
...
# ╔═╡ fd3456ef-3456-abcd-ef01-890123456789
println("L-BFGS optimization completed. Final loss: $(losses_ude[end])")

# ╔═╡ a3456789-bcde-f012-6789-123456789012
# Plot training loss
plot_loss = plot(losses_ude,
...
# Visualize the trained UDE predictions
prob_trained = ODEProblem(ude_f!, X_data[:, 1], (0.0, 5.0), p_trained)

# ╔═╡ ae456ff0-4567-bcde-f012-901234567890
sol_trained = solve(prob_trained, Tsit5(), saveat=0.01)

# ╔═╡ bf567001-5678-cdef-0123-012345678901
plot_trained = plot(sol_trained,
                   label=["UDE Prey" "UDE Predator"],
                   title="Trained UDE vs True Dynamics",
                   linewidth=2)

# ╔═╡ c0678112-6789-def0-1234-123456789012
# Add true solution for comparison
prob_true_extended = ODEProblem(lotka_volterra_true!, u0_true, (0.0, 5.0), p_true)

# ╔═╡ d1789223-789a-ef01-2345-234567890123
sol_true_extended = solve(prob_true_extended, Tsit5(), saveat=0.01)

# ╔═╡ e289a334-89ab-f012-3456-345678901234
plot!(plot_trained, sol_true_extended,
      linestyle=:dash,
      label=["True Prey" "True Predator"],
      linewidth=2)

# ╔═╡ f39ab445-9abc-0123-4567-456789012345
scatter!(plot_trained, t_data, X_data',
        label=["Training Data Prey" "Training Data Predator"],
        markersize=3)
...
Now that we have a trained neural network that captures the missing dynamics, we can use symbolic regression to find interpretable equations that match what the neural network learned.
"""

# ╔═╡ d6789012-ef01-2345-9abc-456789012345
# Generate data from the neural network's learned dynamics
t_symbolic = range(0.0, 3.0, length=300)

# ╔═╡ a4abc556-abcd-1234-5678-567890123456
prob_nn_data = ODEProblem(ude_f!, X_data[:, 1], (0.0, 3.0), p_trained)

# ╔═╡ b5bcd667-bcde-2345-6789-678901234567
sol_nn_data = solve(prob_nn_data, Tsit5(), saveat=t_symbolic)

# ╔═╡ c6cde778-cdef-3456-789a-789012345678
# Calculate the neural network contributions
nn_contributions = zeros(2, length(t_symbolic))

# ╔═╡ d7def889-def0-4567-89ab-890123456789
for (i, t) in enumerate(t_symbolic)
    u_point = sol_nn_data[:, i]
    nn_out = U_nn(u_point, p_trained, nn_states)[1]
    nn_contributions[:, i] = nn_out
end

# ╔═╡ e8ef099a-ef01-5678-9abc-901234567890
# Prepare data for symbolic regression
X_symbolic = sol_nn_data[:, :]

# ╔═╡ f9f01aab-f012-6789-abcd-012345678901
# Calculate derivatives for the neural network part only
# (subtracting known dynamics)
du_symbolic = similar(nn_contributions)

# ╔═╡ aa012bbc-0123-789a-bcde-123456789012
for i in 1:size(X_symbolic, 2)
    x, y = X_symbolic[:, i]
    # These are the NN contributions we want to discover
    du_symbolic[1, i] = nn_contributions[1, i]
    du_symbolic[2, i] = nn_contributions[2, i]
end

# ╔═╡ e7890123-f012-3456-abcd-567890123456
# Build basis functions for symbolic regression
...
# Polynomial and interaction basis
basis = Basis(polynomial_basis(u_vars, 2), u_vars)

# ╔═╡ dd345eef-3456-abcd-ef01-456789012345
# Perform symbolic regression with sparsity
λ_sparse = 1e-2

# ╔═╡ ee456ff0-4567-bcde-f012-567890123456
opt_sr = SR3(λ_sparse, 1.0)
...
discovered_eq1 = sum(discovered_eqs[:, 1] .* basis.basis)

# ╔═╡ e4abc556-abcd-1234-5678-123456789012
println(discovered_eq1)

# ╔═╡ f5bcd667-bcde-2345-6789-234567890123
println("\nMissing term for dy/dt:")

# ╔═╡ a6cde778-cdef-3456-789a-345678901234
discovered_eq2 = sum(discovered_eqs[:, 2] .* basis.basis)

# ╔═╡ b7def889-def0-4567-89ab-456789012345
println(discovered_eq2)
...
println("True missing term for dx/dt: -$(p_true[2]) * x * y")

# ╔═╡ ea012bbc-0123-789a-bcde-789012345678
println("True missing term for dy/dt: $(p_true[4]) * x * y")

# ╔═╡ f8901234-0123-4567-bcde-678901234567
# Visualize the discovered vs true missing terms
x_test = range(0.1, 5.0, length=50)

# ╔═╡ fb123ccd-1234-89ab-cdef-890123456789
y_test = range(0.1, 5.0, length=50)

# ╔═╡ ac234dde-2345-9abc-def0-901234567890
# Calculate true missing physics
true_missing1 = zeros(length(x_test), length(y_test))

# ╔═╡ bd345eef-3456-abcd-ef01-012345678901
true_missing2 = zeros(length(x_test), length(y_test))

# ╔═╡ ce456ff0-4567-bcde-f012-123456789012
for (i, x) in enumerate(x_test)
    for (j, y) in enumerate(y_test)
        true_missing1[i, j] = -p_true[2] * x * y
        true_missing2[i, j] = p_true[4] * x * y
    end
end

# ╔═╡ df567001-5678-cdef-0123-234567890123
# Calculate NN-learned missing physics
nn_missing1 = zeros(length(x_test), length(y_test))

# ╔═╡ e0678112-6789-def0-1234-345678901234
nn_missing2 = zeros(length(x_test), length(y_test))

# ╔═╡ f1789223-789a-ef01-2345-456789012345
for (i, x) in enumerate(x_test)
    for (j, y) in enumerate(y_test)
        u_point = [x, y]
        nn_out = U_nn(u_point, p_trained, nn_states)[1]
        nn_missing1[i, j] = nn_out[1]
        nn_missing2[i, j] = nn_out[2]
    end
end

# ╔═╡ a289a334-89ab-f012-3456-567890123456
# Create comparison plots
p1_true = heatmap(x_test, y_test, true_missing1',
                 title="True Missing: dx/dt",
                 xlabel="x (prey)", ylabel="y (predator)",
                 color=:viridis)

# ╔═╡ b39ab445-9abc-0123-4567-678901234567
p1_nn = heatmap(x_test, y_test, nn_missing1',
               title="NN Learned: dx/dt",
               xlabel="x (prey)", ylabel="y (predator)",
               color=:viridis)

# ╔═╡ c4abc556-abcd-1234-5678-789012345678
p2_true = heatmap(x_test, y_test, true_missing2',
                 title="True Missing: dy/dt",
                 xlabel="x (prey)", ylabel="y (predator)",
                 color=:viridis)

# ╔═╡ d5bcd667-bcde-2345-6789-890123456789
p2_nn = heatmap(x_test, y_test, nn_missing2',
               title="NN Learned: dy/dt",
               xlabel="x (prey)", ylabel="y (predator)",
               color=:viridis)

# ╔═╡ e6cde778-cdef-3456-789a-901234567890
plot_comparison = plot(p1_true, p1_nn, p2_true, p2_nn,
                      layout=(2, 2),
                      size=(800, 600))
...
1. **Data Generation**: Created synthetic observations from the true Lotka-Volterra system with added noise
2. **Hybrid Modeling**: Built a UDE that combines partial knowledge (exponential growth/decay) with a neural network to learn missing interaction terms
3. **Training**: Used a two-stage optimization (ADAM + L-BFGS) to train the neural network
4. **Discovery**: Applied symbolic regression to extract interpretable equations from the trained neural network

The UDE approach successfully discovered that the missing physics corresponds to the predator-prey interaction terms `βxy` and `γxy`, demonstrating how machine learning can "autocomplete" scientific models by identifying missing dynamics from data.

This technique bridges the gap between:
- **Pure mechanistic modeling** (requires complete knowledge)
- **Pure machine learning** (lacks interpretability)

By combining both approaches, we get models that are both accurate and scientifically interpretable.
"""
...
DataDrivenDiffEq = "~1.8.0"
DataDrivenSparse = "~0.1.3"
Lux = "~1.16.0"
Optimization = "~4.7.0"
...
manifest_format = "2.0"
project_hash = "58c88b87c5b2eb66ffbff1c16f0229d32ac9711c"
...
git-tree-sha1 = "a663788ce032449323ec8a9c0dd630e04b9a5b9d"
...
git-tree-sha1 = "5750d63cbfaab85d8eb7e2a3a7c45ac298c01e7d"
...
git-tree-sha1 = "c05e5c3dca1810e728429771763b485057322ad2"
...
git-tree-sha1 = "a3c5d3485c6cdf0c13d81678b9c82a6f4e5e4eca"
